...
from bindu.server.metrics import get_metrics, PrometheusMetrics
...
def test_metrics_record_http_request(metrics):
    ...
    assert 'http_requests_total{method="GET",endpoint="/health",status="200"} 2' in output
...
def test_metrics_agent_tasks(metrics):
    ...
    assert f'agent_tasks_completed_total{{agent_id="{agent_id}",status="success"}} 2' in output
    assert f'agent_tasks_completed_total{{agent_id="{agent_id}",status="failed"}} 1' in output
...
def test_metrics_thread_safety(metrics):
    ...
    output = metrics.generate_prometheus_text()
    assert 'http_requests_total{method="GET",endpoint="/test",status="200"} 50' in output
